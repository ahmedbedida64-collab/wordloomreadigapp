...
let selectedFilter = 'all'; let selectedTopic = 'all'; let selectedLevel = 'all'; let libraryQuery = ''; let selectedVocabLevel = 'all'; let motherLanguage = localStorage.getItem('wordloom-language') || 'ar'; let learnerLevel = localStorage.getItem('wordloom-level') || 'A1'; let graphicsEnabled = localStorage.getItem('wordloom-graphics') !== 'off'; let savedWords = new Set(JSON.parse(localStorage.getItem('wordloom-saved-words') || '[]')); let vocabularyRecords = JSON.parse(localStorage.getItem('wordloom-vocabulary-records') || '{}'); let activeWordContext = '';
...
function wordKey(text) { return text.toLowerCase().replace(/[^a-z]/g, ''); }
...
function updateReadingStats() {
  document.querySelector('#stats-minutes').textContent = readingMetrics.minutes || 0;
  document.querySelector('#stats-lookups').textContent = readingMetrics.lookups || 0;
  document.querySelector('#stats-streak').textContent = readingMetrics.streak || 0;
}
...
function renderReaderState() {
  ...
  storyCopy.innerHTML = `<div class="reader-page" data-page="${page.pageNumber}"><h3>${page.subtitle}</h3><div class="page-graphic" aria-label="Visual guide"><span class="graphic-symbol">${graphic[2]}</span></div>${page.paragraphs.map(paragraph => `<p>${buildWordMarkup(paragraph)}</p>`).join('')}<span class="page-number">Page ${page.pageNumber} of ${pages.length}</span></div>`;
...
function closeModals() { document.querySelectorAll('.modal-backdrop').forEach(modal => { modal.classList.add('hidden'); modal.setAttribute('hidden', ''); }); }
function showDictionary(word) {
  ...
  const key = word?.dataset?.word?.trim().toLowerCase();
  ...
  const translation = customTranslations[key]?.[motherLanguage] || (motherLanguage === 'ar' ? data[0] : `${data[0]} (${translations[motherLanguage]})`);
  activeWordContext = decodeURIComponent(word.dataset.context || data[3]);
  ...
  updateReadingStats();
  vocabularyRecords[key] = vocabularyRecords[key] || { word: key, meaning: data[1], level: 'B1', sentence: activeWordContext, repetitions: 0, interval: 0, nextReview: Date.now() };
  vocabularyRecords[key].sentence = activeWordContext;
  ...
  content.innerHTML = `<span class="detail-label">WORD IN CONTEXT</span><h3 id="dictionary-title">${key}</h3><span class="word-pronounce">/${key}/ · ${data[2]}</span><p class="word-definition">${data[1]}</p><div class="translation-line"><span>Translation · ${translations[motherLanguage]}</span><strong>${translation}</strong></div><div class="context-box">“${activeWordContext}”</div><div class="modal-actions"><button class="speak-word" data-speak="${key}" type="button">▶ Hear clearly</button><button class="modal-save" data-save-word="${key}" type="button">✓ Saved to garden</button></div>`;
...
function showFullDictionary(key) { const modal = document.querySelector('#dictionary-modal'); const content = document.querySelector('#dictionary-content'); const data = dictionary[key] || [key, 'A useful word from this story.', 'word', `The word “${key}” appears in this story.`]; const translation = getWordTranslation(key, data); content.innerHTML = `<span class="detail-label">WORD IN CONTEXT</span><h3 id="dictionary-title">${key}</h3><span class="word-pronounce">/${key}/ · ${data[2]}</span><p class="word-definition">${data[1]}</p><div class="translation-line"><span>Translation · ${translations[motherLanguage]}</span><strong>${translation}</strong></div><div class="detail-label" style="display:block;margin-top:22px">ANOTHER EXAMPLE</div><div class="modal-example">${activeWordContext || data[3]}</div><div class="modal-actions"><button class="speak-word" data-speak="${key}" type="button">◉ Hear word</button><button class="modal-save" data-save-word="${key}" type="button">${savedWords.has(key) ? '✓ Saved' : '+ Save to vocabulary'}</button></div>`; modal.removeAttribute('hidden'); modal.classList.remove('hidden'); document.querySelector('#quick-word-popover')?.classList.add('hidden'); }
function savePreferences() { motherLanguage = document.querySelector('#mother-language').value; learnerLevel = document.querySelector('#reader-level').value; graphicsEnabled = document.querySelector('#book-graphics').checked; const mode = document.querySelector('#color-mode-setting').value; localStorage.setItem('wordloom-language', motherLanguage); localStorage.setItem('wordloom-level', learnerLevel); localStorage.setItem('wordloom-graphics', graphicsEnabled ? 'on' : 'off'); localStorage.setItem('wordloom-mode', mode); document.body.classList.toggle('theme-dark', mode === 'dark'); document.body.classList.toggle('graphics-off', !graphicsEnabled); renderReaderState(); closeModals(); showToast(`Settings saved · ${learnerLevel}`); }
function renderVocabulary(query = '') { const matches = vocabulary.filter(([word, meaning, level]) => (selectedVocabLevel === 'all' || level === selectedVocabLevel) && `${word} ${meaning}`.toLowerCase().includes(query.toLowerCase())); const groups = ['A2', 'B1', 'B2'].map(level => { const words = matches.filter(item => item[2] === level); return words.length ? `<section class="word-group"><div class="word-group-heading"><h3>${level} words</h3><span>${words.length} saved</span></div><div class="word-list">${words.map(([word, meaning]) => { const record = vocabularyRecords[word] || { sentence: 'Save a word while reading to keep its sentence here.', repetitions: 0, nextReview: Date.now() }; const due = record.nextReview <= Date.now(); return `<article class="word-row${due ? ' review-due' : ''}"><strong>${word}</strong><span><em>${meaning}</em><small class="saved-sentence">${record.sentence}</small></span><span class="word-level">${level}</span><button class="review-word" data-review-word="${word}" type="button">${due ? 'Review' : 'Due later'}</button></article>`; }).join('')}</div></section>` : ''; }).join(''); document.querySelector('#word-list').innerHTML = groups || '<p class="empty-state">No vocabulary matches your search.</p>'; document.querySelector('#saved-word-count').textContent = savedWords.size || vocabulary.length; document.querySelector('.nav-item[data-view="vocabulary"] .nav-count').textContent = savedWords.size || vocabulary.length; }
function reviewWord(word, rating) { const record = vocabularyRecords[word] || { word, repetitions: 0, interval: 0, nextReview: Date.now() }; const factors = { Again: [0, 1], Hard: [Math.max(1, record.repetitions), Math.max(1, Math.round((record.interval || 1) * 1.5))], Good: [record.repetitions + 1, Math.max(1, Math.round((record.interval || 1) * 2.5))], Easy: [record.repetitions + 1, Math.max(2, Math.round((record.interval || 1) * 3.5))] }; const [repetitions, interval] = factors[rating]; record.repetitions = repetitions; record.interval = interval; record.nextReview = Date.now() + interval * 86400000; vocabularyRecords[word] = record; localStorage.setItem('wordloom-vocabulary-records', JSON.stringify(vocabularyRecords)); renderVocabulary(document.querySelector('#vocab-search').value); showToast(`${word} reviewed · next review in ${interval} day${interval === 1 ? '' : 's'}`); }
...
document.addEventListener('click', event => { const word = event.target.closest('.word'); if (word) { showDictionary(word); return; } const more = event.target.closest('[data-more-word]'); if (more) { showFullDictionary(more.dataset.moreWord); return; } const save = event.target.closest('[data-save-word]'); if (save) { savedWords.add(save.dataset.saveWord); localStorage.setItem('wordloom-saved-words', JSON.stringify([...savedWords])); save.textContent = '✓ Saved'; renderVocabulary(document.querySelector('#vocab-search').value); showToast(`${save.dataset.saveWord} saved to your vocabulary`); return; } const speak = event.target.closest('[data-speak]'); if (speak && 'speechSynthesis' in window) { speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(speak.dataset.speak); utterance.lang = 'en-US'; utterance.rate = .78; utterance.pitch = 1; speechSynthesis.speak(utterance); } const review = event.target.closest('[data-review-word]'); if (review) { const wordRecord = vocabularyRecords[review.dataset.reviewWord] || {}; const rating = window.prompt(`Recall “${review.dataset.reviewWord}”\n\n${wordRecord.sentence || 'No saved sentence yet.'}\n\nType: Again, Hard, Good, or Easy`, 'Good'); if (rating && ['Again', 'Hard', 'Good', 'Easy'].includes(rating)) reviewWord(review.dataset.reviewWord, rating); } const answer = event.target.closest('[data-answer]'); if (answer) { const feedback = document.querySelector('.comprehension-feedback'); feedback.textContent = answer.dataset.answer === 'main' ? 'Correct. You identified the central idea.' : 'Keep reading for the connected explanation.'; } if (event.target.closest('[data-skip-comprehension]')) document.querySelector('#comprehension-panel').classList.add('hidden'); });
...
document.querySelectorAll('.level-button').forEach(button => button.addEventListener('click', () => { document.querySelectorAll('.level-button').forEach(item => { item.classList.remove('active'); item.setAttribute('aria-pressed', 'false'); }); button.classList.add('active'); button.setAttribute('aria-pressed', 'true'); selectedLevel = button.dataset.level; renderBooks(); }));
...
document.querySelector('#export-vocabulary').addEventListener('click', exportVocabulary);
updateReadingStats();
