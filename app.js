...
updateFlowMetric();
document.querySelectorAll('.nav-item').forEach(button => button.addEventListener('click', () => button.dataset.view === 'reading' ? openReader() : showView(button.dataset.view))); document.querySelector('#continue-reading').addEventListener('click', () => openReader('The Lighthouse')); document.querySelector('#back-to-library').addEventListener('click', () => showView('library')); document.querySelector('#import-pdf').addEventListener('click', () => document.querySelector('#pdf-input').click()); document.querySelector('#add-book').addEventListener('click', () => document.querySelector('#pdf-input').click()); document.querySelector('#pdf-input').addEventListener('change', event => { const file = event.target.files[0]; if (file) showToast(`${file.name} added. PDF reader setup is next.`); event.target.value = ''; });
