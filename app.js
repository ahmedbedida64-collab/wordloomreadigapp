...
function renderReaderState() {
  ...
  storyCopy.innerHTML = `<div class="reader-page" data-page="${page.pageNumber}"><h3>${page.subtitle}</h3><div class="page-graphic"><div><span class="graphic-symbol">${graphic[2]}</span>${graphic[0]} · visual guide</div></div>${page.paragraphs.map(paragraph => `<p>${buildWordMarkup(paragraph)}</p>`).join('')}<span class="page-number">Page ${page.pageNumber} of ${pages.length}</span></div>`;
