...
function showDictionary(word) { const key = word.dataset.word; const data = dictionary[key] || [key, 'a word used in this story', 'word', `The word “${key}” appears in this story.`]; const translation = customTranslations[key]?.[motherLanguage] || (motherLanguage === 'ar' ? data[0] : `${data[0]} (${translations[motherLanguage]})`); activeWordContext = decodeURIComponent(word.dataset.context || data[3]); readingMetrics.lookups = (readingMetrics.lookups || 0) + 1; localStorage.setItem('wordloom-reading-metrics', JSON.stringify(readingMetrics)); updateReadingStats(); vocabularyRecords[key] = vocabularyRecords[key] || { word: key, meaning: data[1], level: 'B1', sentence: activeWordContext, repetitions: 0, interval: 0, nextReview: Date.now() }; vocabularyRecords[key].sentence = activeWordContext; vocabularyRecords[key].meaning = data[1]; localStorage.setItem('wordloom-vocabulary-records', JSON.stringify(vocabularyRecords)); savedWords.add(key); localStorage.setItem('wordloom-saved-words', JSON.stringify([...savedWords])); document.querySelector('#dictionary-content').innerHTML = `<span class="detail-label">WORD IN CONTEXT</span><h3 id="dictionary-title">${key}</h3><span class="word-pronounce">/${key}/ · ${data[2]}</span><p class="word-definition">${data[1]}</p><div class="translation-line"><span>Translation · ${translations[motherLanguage]}</span><strong>${translation}</strong></div><div class="context-box">“${activeWordContext}”</div><div class="modal-actions"><button class="speak-word" data-speak="${key}" type="button">▶ Hear clearly</button><button class="modal-save" data-save-word="${key}" type="button">✓ Saved to garden</button></div>`; document.querySelector('#dictionary-modal').classList.remove('hidden'); document.querySelector('#quick-word-popover').classList.add('hidden'); }
