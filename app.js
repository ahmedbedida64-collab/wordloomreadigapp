...
function showDictionary(word) {
  ...
  const key = word?.dataset?.word ? wordKey(word.dataset.word) : '';
...
function savePreferences() { motherLanguage = document.querySelector('#mother-language').value; learnerLevel = document.querySelector('#reader-level').value; const mode = document.querySelector('#color-mode-setting').value; localStorage.setItem('wordloom-language', motherLanguage); localStorage.setItem('wordloom-level', learnerLevel); localStorage.setItem('wordloom-mode', mode); document.body.classList.toggle('theme-dark', mode === 'dark'); renderReaderState(); closeModals(); showToast(`Settings saved · ${learnerLevel}`); }
...
document.querySelector('#mother-language').value = motherLanguage; document.querySelector('#reader-level').value = learnerLevel; document.querySelector('#color-mode-setting').value = savedMode; document.body.classList.toggle('theme-dark', savedMode === 'dark');
updateFlowMetric();
